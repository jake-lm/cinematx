<?php
// ═══════════════════════════════════════════════════════════════════════════
//  CINEMA, TX — v6 front page
//  Modern app register. Rail + bar as quiet chrome, a canvas where the lead
//  essay is the only loud thing and everything else is a small quiet card.
//  /index.php and the earlier attempts are untouched.
// ═══════════════════════════════════════════════════════════════════════════
error_reporting(0);
session_start();
...
$q = $conn->prepare("SELECT p.id, p.uid, p.title, p.type, p.stamp, p.edited, u.name AS author_name
  ...
  "ORDER BY COALESCE(p.edited, p.stamp) DESC LIMIT 6");
...
function show_row($s, $now, $e) {
  $member = ($s['source'] ?? '') === 'user';
  $href   = !empty($s['url']) ? $s['url'] : '/list';
  $other  = date('j M', $s['timestamp']) !== date('j M', $now);
  ?>
  <a class="show<?php echo $member ? ' show--member' : ''; ?>" href="<?php echo $e($href); ?>"<?php echo $member ? '' : ' target="_blank" rel="noopener"'; ?>>
    <span class="show__time"><?php echo date('g:ia', $s['timestamp']); ?></span>
    <span class="show__main">
      <span class="show__title"><?php echo $e($s['title']); ?></span>
      <span class="show__where">
        <?php if ($member): ?><span class="show__by">&#9679; By a member</span> &middot; <?php endif; ?>
        <?php echo $e($s['venue']); ?><?php echo $other ? ' · ' . date('D', $s['timestamp']) : ''; ?>
      </span>
    </span>
  </a>
  <?php
...
<script>try{var t=localStorage.getItem('ctx-theme');if(t==='light')document.documentElement.setAttribute('data-theme','light');}catch(e){}</script>
<link rel="stylesheet" href="/css/v6.css?v=<?php echo filemtime(dirname(__DIR__) . '/css/v6.css'); ?>" />
...
<script src="/js/v6.js?v=<?php echo filemtime(dirname(__DIR__) . '/js/v6.js'); ?>" defer></script>
</head>
<body>
...
<div class="app" id="app">
  ...
  <aside class="rail">
    <div class="rail__top">
      <a class="rail__mark" href="/v6/">Cinema<span class="c">,</span> TX</a>
    </div>

    <nav class="rail__nav">
      <a class="rail__link is-on" href="/v6/"><span class="ico"><i class="fa-solid fa-newspaper"></i></span><span class="txt">Index</span></a>
      <a class="rail__link" href="/list"><span class="ico"><i class="fa-solid fa-film"></i></span><span class="txt">The List</span></a>
  ...
  <header class="bar">
    ...
    <a class="bar__mark" href="/v6/">Cinema<span class="c">,</span> TX</a>
    <span class="welcome" id="welcome">Welcome to the Cinema</span>

    <div class="bar__end">
      <button class="ibtn" id="theme" title="Light"><i class="fa-solid fa-sun"></i></button>
      <?php if ($signed): ?>
  ...
  <main class="canvas">
    <div class="canvas__in">
      ...
      <!-- ── The journal — the one loud thing ─────────────────────────── -->
      <section class="card">
        ...
        <div class="card__body">
          ...
          <div class="strip">
            ...
            <a class="row" href="<?php echo $e(post_url($p['id'], $p['title'])); ?>">
              ...
              <span class="row__meta"><?php echo $e($p['author_name'] ?: 'Member'); ?> &middot; <?php echo date('j M', $p['edited'] ?: $p['stamp']); ?></span>
            </a>
            <?php endforeach; ?>
          </div>
        </div>
      ...
      <div class="side">
        ...
        <div class="screen" id="screen">
          ...
          <div class="screen__meta">
            <?php if ($is_live): ?>
              <span class="screen__kicker"><span class="dot"></span> On screen now</span>
            <?php else: ?>
            ...
            <?php if ($film): ?>
            <span class="screen__sub"><?php echo $e($film['director']); ?> &middot; Everyone watches at once</span>
            <?php endif; ?>
          </div>
        </div>
        ...
        <section class="card">
          <div class="card__head">
            ...
            <span class="card__title"><?php echo $e($label); ?></span>
            <button class="card__more" data-open="list">All <?php echo count($screenings); ?> &rarr;</button>
          </div>
          <div class="card__body">
            <?php if ($screenings): foreach (array_slice($screenings, 0, 10) as $s) show_row($s, $now, $e); ?>
            <?php else: ?><p class="empty">Nothing listed</p><?php endif; ?>
          </div>
        </section>
...
<div class="sheet" id="sheet-list">
  ...
  <div class="sheet__body" style="padding:0;">
    <?php foreach ($screenings as $s) show_row($s, $now, $e); ?>
  </div>
</div>
