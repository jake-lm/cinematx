// ═══════════════════════════════════════════════════════════════════════════
//  CINEMA, TX — v5 behaviour
//  Vanilla. Unchanged PHP endpoints. theatre_1()'s sync algorithm carried over.
//
//  The page never scrolls, so depth is reached through overlays that blur the
//  front page behind them — you never lose your place.
// ═══════════════════════════════════════════════════════════════════════════
...
(function () {
  ...
  // ══ Overlays ═════════════════════════════════════════════════════════════
  // One controller for every overlay: [data-open="x"] shows #sheet-x.
  ...
  function initOverlays() {
    ...
    function close() {
      if (openSheet) openSheet.classList.remove('is-on');
    ...
    function open(name) {
      var el = $('#sheet-' + name);
      if (!el) return;
      if (openSheet && openSheet !== el) openSheet.classList.remove('is-on');
    ...
    if (location.hash === '#join') open('account');
  }

  // ══ Head: mobile nav ═════════════════════════════════════════════════════

  function initHead() {
    var head = $('#head'), tog = $('#head-toggle');
    if (head && tog) tog.addEventListener('click', function (e) {
      e.stopPropagation();
  ...
  function initTheatre() {
    ...
    function close() { shell.classList.remove('is-on'); document.body.style.overflow = ''; }
  ...
  function boot() {
    initTheme(); initHead(); initOverlays();
