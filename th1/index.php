...
require '../database.php';
  ...
  $sql_now = $conn->query("SELECT * FROM `showtimes` WHERE $now > `showtime` AND $now < `endtime` AND `theatre` = 1");
  $showtime = $sql_now->fetch();

  $sql_next = $conn->query("SELECT * FROM `showtimes` WHERE $now < `showtime` AND `theatre` = 1 LIMIT 1");
  ...
  $sql3->execute();
  $films_count = $conn->query("SELECT count(*) FROM `showtimes` WHERE `endtime` > $now AND `showtime` < $tomorrow AND `theatre` = 1")->fetchColumn();
  ...
  if(! $showtime) {
    if(! $showtime_next) {
      ...
      $sql = $conn->prepare("SELECT * FROM `showtimes` WHERE $now < `showtime` AND `theatre` = 1 ORDER BY `showtime` ASC LIMIT 1");
      $sql->execute();
      $showtime = $sql->fetch();
      $f_id = $showtime['f_id'];
    ...
    $sql = $conn->prepare("SELECT * FROM `showtimes` WHERE $now > `showtime` AND $now < `endtime` AND `theatre` = 1 LIMIT 1");
    $sql->execute();
    $showtime = $sql->fetch();
    $f_id = $showtime['f_id'];
    $sql2 = $conn->prepare("SELECT * FROM `films` WHERE `id` = $f_id");
...
<head>
  ...
  <meta property="og:description" content="<?php echo $note['note']; ?>" />
...
<div class="main-content">
  ...
  <div class="home-base">
    ...
    <div class="video-hold" id="video-hold"><video
      id="motw"
      class="video-js"
      ...
      <div class="video-info">
        <div class="topbar">
          <span class="title">
            <?php
              if(isset($film['title'])){
                echo '<a href="'.$film['wiki'].'" target="_blank">'.$film['title']. '</a> - ' . $showtime2 . '';
              }
              else {
